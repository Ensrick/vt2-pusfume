...
The command writes only beneath ignored `.build/native-workshop`, generates VT2 texture recipes and materials from the untracked handoff, enables the native cosmetic in that staged copy, compiles it with VMB, and copies the resulting bundles into Workshop item `3764954245` by default. Use `-HeroPreview` only for an intentional 3D selector test and `-NoDeploy` only for CI or a compiler-only check. The tracked config intentionally leaves native mode and native preview disabled so a normal public build cannot reference an absent or unreviewed model.
...
Autodesk's [character setup workflow](https://help.autodesk.com/cloudhelp/ENU/Stingray-Help/stingray_help/animation/set_up_character.html) requires animation clips to target the same skeleton used by the character unit, and its [controller documentation](https://help.autodesk.com/cloudhelp/ENU/Stingray-Help/stingray_help/animation/animation_controllers.html) requires the controller to be associated with that skeleton. Pusfume's compiled artifacts satisfy the static side of this contract: the unit skeleton, clip sync pose, and clip tracks each contain the same 82 bones; the walk clip is 0.8 seconds and contains 1,109 keyed samples.

At runtime, a valid controller is not sufficient proof of deformation. Stingray's [Unit animation API](https://help.autodesk.com/cloudhelp/ENU/Stingray-Help/lua_ref/obj_stingray_Unit.html) allows evaluated animation to be withheld from bone nodes when the unit's animation bone mode is `ignore`, and bone LOD can freeze unevaluated bones. The native integration therefore sets bone mode to `transform`, selects bone LOD 0, enables the state machine, and logs layer playhead time alongside skeletal articulation.

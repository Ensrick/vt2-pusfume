...
-- Baked per-bone pose data for Janfon's Assassin clips (generated by
-- tools/bake_animation_poses.py from the compiled eye-frame clips; FK-gated
-- at bake time). Engine playback re-anchors this unit at the world origin,
-- so the runtime replays these poses directly (issue #46).
local assassin_poses =
    mod:dofile("scripts/mods/pusfume/pusfume_assassin_poses")
...
local function play_custom_first_person_clip(extension, event_name)
    ...
    if not pose_clip then
        if not extension._pusfume_assassin_event_gap_logged then
            ...
            mod:info("[pusfume] Assassin pose data missing clip=%s", event_name)
        end
        return false
    end
    ...
    if not extension._pusfume_assassin_manual_driver then
        ...
        mod:info("[pusfume] Janfon assassin pose player enabled")
    end

    -- Resolve and cache node indices once per unit, and apply the clip's
    -- static local positions at clip start.
    local node_cache = extension._pusfume_assassin_node_cache
    if not node_cache or node_cache.unit ~= animation_unit then
        node_cache = { unit = animation_unit, nodes = {} }
        for bone_name in pairs(pose_clip.bones) do
            if Unit.has_node(animation_unit, bone_name) then
                node_cache.nodes[bone_name] = Unit.node(animation_unit, bone_name)
            end
        end
        extension._pusfume_assassin_node_cache = node_cache
    end
    for bone_name, bone_data in pairs(pose_clip.bones) do
        local node = node_cache.nodes[bone_name]
        if node and node ~= 0 then
            Unit.set_local_position(animation_unit, node,
                Vector3(bone_data.p[1], bone_data.p[2], bone_data.p[3]))
        end
    end

    local target_duration = ASSASSIN_CLIP_TARGET_DURATION[event_name]
        or pose_clip.duration
    extension._pusfume_assassin_clip = {
        animation_unit = animation_unit,
        event = event_name,
        duration = pose_clip.duration,
        loop = clip.loop == true,
        next_sample = 0.1,
        playback_rate = pose_clip.duration / target_duration,
...
local function update_custom_first_person_clip(extension, t)
    ...
    -- Pose player: sample each bone's rotation keys at clip_time and write
    -- the locals directly. Keys are {t, x, y, z, w}, sorted by time; nlerp
    -- between the bracketing pair.
    local node_cache = extension._pusfume_assassin_node_cache
    if active.pose_bones and node_cache
            and node_cache.unit == active.animation_unit then
        for bone_name, bone_data in pairs(active.pose_bones) do
            local node = node_cache.nodes[bone_name]
            if node and node ~= 0 then
                local keys = bone_data.k
                local count = #keys
                local sampled = keys[count]
                if clip_time <= keys[1][1] then
                    sampled = keys[1]
                    Unit.set_local_rotation(active.animation_unit, node,
                        Quaternion.from_elements(
                            sampled[2], sampled[3], sampled[4], sampled[5]))
                else
                    for index = 2, count do
                        local key = keys[index]
                        if key[1] >= clip_time then
                            local previous = keys[index - 1]
                            local span = key[1] - previous[1]
                            local alpha = span > 0
                                and (clip_time - previous[1]) / span or 0
                            local q = Quaternion.lerp(
                                Quaternion.from_elements(previous[2],
                                    previous[3], previous[4], previous[5]),
                                Quaternion.from_elements(key[2], key[3],
                                    key[4], key[5]),
                                alpha)
                            Unit.set_local_rotation(
                                active.animation_unit, node, q)
                            sampled = nil
                            break
                        end
                    end
    ...
    -- directly every frame, because the engine composes this unit's bones
    -- in its own directly-set world transform, then force the composition
    -- through with World.update_unit (the engine's own recipe for units
    -- riding moving transforms; proven live in v0.6.95).
...
local function switch_first_person_rig(extension, inventory_extension, role)
    ...
    if use_skaven then
        -- The arms unit's compiled controller stays permanently DISABLED
        -- (spawn-time disable): enabling it re-anchors every animated bone
        -- at the world origin (v0.6.96), and it would boot into claws_idle
        -- under the per-bone-linked roles (the all-weapons mangled hands
